...
int lg(int n)
{
    ...
    if(n <= 0){
        printf("Treesynth, lg : Sorry, I'm a hack and only compute approximate logs of positive integers");
        exit(2);
    }
...
bool Tree::initialize(TS_UINT levels){
    if(levels < 1){
        printf("Tree::initialize : levels (%i) must be > 1", levels);
        return false;
    }
    
    if(levels > 20){
        printf("Tree::initialize : levels (%i) is too big for me. Retrench!", levels);
        return false;
    }
...
short Treesynth::Predecessors(int level, int offset, int csl){
    ...
    int npred = our_min <int>(npredecessors, offset);
...
bool Treesynth::setup(void){
    ...
    // Wavelet decompositions --  10 is the filter length (4 for same results as daub4, 10 for 'db5' (daub10, 5 vanishing moments))
    pwtset(10);
    ...
    for(lev = startlevel; lev <= stoplevel; lev++)
    {
        ...
        for(int offset = 0; offset < (1 << lev); offset++){     // (int)(pow( 2, lev )
            ...
            if(tnew_data->getNode(lev, offset)->cslength == 0){
                //BB_log( BB_LOG_WARNING, "Treesynth: Empty candidate set in level %i, offset%i!!", lev, offset );
            }
...
void Treesynth::resetTreeLevels(TS_UINT lev){
    if(lefttree){
        printf(" Treesynth: Sorry, cannot reset levels because lefttree will get messed up");
        return;
    }
...
TreesynthIO::TreesynthIO(){
    ...
    printf("TreesynthIO: You have created a TreesynthIO object\n \
           Input file name - %s ; change by setting ifilename\n \
           Output file name - %s ; change by setting ofilename\n \
           Left file name - %s ; change by setting leftfile\n \
           Read mode - %i ; change by setting rm_mode (see The ReadModes)\n \
           Samples - %i ; change by setting nsamples", ifilename, ofilename, leftfile, \
           rm_mode, nsamples);
}
...
TreesynthIO::~TreesynthIO()
{
    ...
    // AudioCentral::instance()->m_cachemanager->closesf( sfread );  // IOHAVOC -- CALLBACK MECHANISM -
    
    if(sfwrite){
        sf_close(sfwrite);
    }
    // AudioCentral::instance()->m_cachemanager->closesf( sfwrite );	// IOHAVOC -- CALLBACK, MAIN AUDIO RENDERING INSTANCE?
    
    printf("TreesynthIO : Goodbye!");
}
...
int TreesynthIO::m_audio_cb(char * buf, int buffer_size, void * user_data){
    ...
    while(remaining > 0){
        ...
        int tocopy = (int)our_min <long>(remaining, g_audio_end - g_audio_begin);
...
void TreesynthIO::set_next_pos(const char * filename){
    ...
    switch(direction){
        ...
        default:
            //BB_log( BB_LOG_WARNING, "TreesynthIO::set_next_pos - invalid read direction, exiting" );
            exit(1);
    }
    ...
    if(rm_next_pos >= readinfo.frames){
        switch(action){
            case RM_STOP: {
                //BB_log( BB_LOG_INFO, "TreesynthIO::set_next_pos : cannot read no mo' of file '%s', stopping", filename );
                char x[256];
                cin.getline(x, 256);
                exit(1);
                break;
            ...
            case RM_WRAP:
                //BB_log( BB_LOG_INFO, "TreesynthIO::set_next_pos : wrapping" );
                rm_next_pos = 0;
                break;
                
            case RM_BOUNCE:
                //BB_log( BB_LOG_INFO, "TreesynthIO::set_next_pos : bouncing" );
            ...
            default:
                //BB_log( BB_LOG_WARNING, "TreesynthIO::set_next_pos - invalid action, exiting" );
                exit(1);
        }
    }
        ...
        switch(action){
            ...
            case RM_WRAP:
                //BB_log( BB_LOG_INFO, "TreesynthIO::set_next_pos : wrapping" );
                rm_next_pos = (int)readinfo.frames - (readinfo.frames % rm_next_length);
                break;
...
int TreesynthIO::ReadSoundFile(char filename[], TS_FLOAT * data, int datasize){
    if(!sfread){
        ...
        //		sfread = AudioCentral::instance()->m_cachemanager->opensf( filename, SFM_READ, &readinfo );  // IOHAVOC - DO FILE I/O INSTEAD
        
        if(!sfread){
            printf("TreesynthIO::ReadSoundFile : cannot open file '%s', quitting", filename);
            //char x[256];
    ...
    int itemsread = (int)sf_read_float(sfread, data, datasize);
    set_next_pos(filename);
    
    // to keep size consistent, ie if end segment is too short, read again
    if(itemsread < datasize && itemsread < readinfo.frames){
        printf("Requested: %d, got: %d, available: %d; trying again \n\n", datasize, itemsread, (int)readinfo.frames);
        // return this->ReadSoundFile(filename, data, datasize);
...
int TreesynthIO::WriteSoundFile(char filename[], TS_FLOAT * data, int datasize){
    ...
    if(this->write_to_buffer){
        // IOHAVOC --turning off writing limit num buffs == 8. WTH?
    ...
    if(this->write_to_file){
        if(!sfwrite){
            ...
            //			sfwrite = AudioCentral::instance()->m_cachemanager->opensf( filename, SFM_WRITE, &writeinfo );  // IOHAVOC -- WRITE FILE INSTEAD
            
            if(!sfwrite){
                printf("TreesynthIO::WriteSoundFile : cannot open file '%s', quitting", filename);
                //char x[256];
                //std::cin.getline( x, 256 );
                //exit(1);
                return 2;
        ...
        int itemswritten = (int)sf_write_float(sfwrite, data, datasize);
        if(itemswritten <= 0){
            printf("TreesynthIO::WriteSoundFile : cannot write to file '%s', quitting", filename);
            //char x[256];
            //std::cin.getline( x, 256 );
            //exit(3);
            return 2;
