...
class VRAE:
    ...
    def create_gradientfunctions(self,data):
        ...
        x0 = T.matrix("x0")
        [y, _], _ = theano.scan(decodingstep, n_steps = x.shape[0],  outputs_info = [x0, h0_dec])
        ...
        numer = T.sqrt(1 - (1 - self.b2)**epoch).astype(theano.config.floatX)
        denom = (1 - (1 - self.b1)**epoch).astype(theano.config.floatX)
        gamma = numer/denom
    ...
    def encode(self, x):
        ...
        b_xhe = self.params["b_he"].get_value() # b_xhe = self.params["b_xhe"].get_value()
