...
for epoch in xrange(1, n_epochs + 1):
    ...
    for i in xrange(0, train_x.shape[0], n_batch):
        ...
        rec_loss, kl_loss, output = model.forward_one_step(x_batch, y_batch, n_layers_recog, n_layers_gen,
                                                           'relu', 'relu', gpu=args.gpu)
    ...
    rec_loss, kl_loss, _ = model.forward_one_step(x_batch, y_batch, n_layers_recog, n_layers_gen,
                                                  'relu', 'relu', gpu=args.gpu)
    t2 = time.time()
    print("\nrec_loss.data: " + str(rec_loss.data) +
          "   kl_loss.data: " + str(kl_loss.data) +
          "   total_loss.data: " + str(total_loss.data))
